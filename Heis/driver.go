...
const( 
	...
	BUTTON_CALL_DOWN
	BUTTON_COMMAND //Endre navn til BUTTON_IN_ELEV eller noe sånt? 
	)
...
func Poll(){
	...
	var count1 = [4]int{0, 0, 0, 0}
	var count2 = [4]int{0, 0, 0, 0}
	count3 := 0

	for{
		for i := 0; i < N_FLOORS; i++{
			if (i != 0 && Get_button_signal(BUTTON_CALL_DOWN, i) != 0 && count[i] == 0){
				//fmt.Println("BUTTON_CALL_DOWN\n")
				count[i] = 1
				New_order <- Order{i, BUTTON_CALL_DOWN}
			}
			if (i != 0 && Get_button_signal(BUTTON_CALL_DOWN, i) == 0 && count[i] != 0){
				//fmt.Println("BUTTON_CALL_DOWN, COUNT = 0\n")
				count[i] = 0
			}
			
			if(i != N_FLOORS - 1 && Get_button_signal(BUTTON_CALL_UP, i) != 0 && count1[i] == 0){
				//fmt.Println("BUTTON_CALL_UP\n")
				count1[i] = 1
				New_order <- Order{i, BUTTON_CALL_UP}	
			}
			if (i != N_FLOORS - 1 && Get_button_signal(BUTTON_CALL_UP, i) == 0 && count1[i] != 0){
				//fmt.Println("BUTTON_CALL_UP, COUNT1 = 0\n")
				count1[i] = 0
			}
			
			if(Get_button_signal(BUTTON_COMMAND, i) != 0 && count2[i] == 0){
				count2[i] = 1
				New_order <- Order{i, BUTTON_COMMAND}
			}
			if (Get_button_signal(BUTTON_COMMAND, i) == 0 && count2[i] != 0){
				count2[i] = 0
			}
